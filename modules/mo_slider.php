...
function mo_slider( $id='slider' ){
	$indicators = '';
	$inner = '';
	$sort = im($id.'_sort') ? im($id.'_sort') : '1 2 3 4 5';
    $sort = array_unique(explode(' ', trim($sort)));
    $i = 0;
    foreach ($sort as $key => $value) {
        if( im($id.'_src_'.$value) && im($id.'_href_'.$value) && im($id.'_title_'.$value) ){
            $indicators .= '<li data-target="#'.$id.'" data-slide-to="'.$i.'"'.(!$i?' class="active"':'').'></li>';
            $inner .= '<div class="item'.(!$i?' active':'').'"><a'.( im($id.'_blank_'.$value) ? ' target="_blank"' : '' ).' href="'.im($id.'_href_'.$value).'"><img src="'.im($id.'_src_'.$value).'"></a></div>';
            // <span class="carousel-caption">'.im($id.'_title_'.$value).'</span>
            $i++;
        }
    }
